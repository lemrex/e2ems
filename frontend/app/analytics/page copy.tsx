...
export default function AnalyticsPage() {
  ...
  const generateSparkles = () => {
    ...
    const fetchCategoryData = async () => {
      ...
      const [expenseRes, incomeRes] = await Promise.all([
        fetch(`${baseURL}/transactions/topex`, {
          headers: { Authorization: `Bearer ${token}` },
      }),
      fetch(`${baseURL}/transactions/topin`, {
        headers: { Authorization: `Bearer ${token}` },
      }),
    ])
    ...
    setExpenseData(
      expenseJson.data.map((item: any, index: number) => ({
        name: item.name,
    ...
    setIncomeData(
      incomeJson.data.map((item: any, index: number) => ({
        name: item.name,
        value: Number(item.value),
        color: VIBRANT_COLORS[index % VIBRANT_COLORS.length],
      }))
  ...
  const fetchAllData = async () => {
    try {
      ...
      const [transactionsRes, weeklyRes, monthlyRes, dailyRes, comparisonRes] = await Promise.all([
        fetch(`${baseURL}/transactions/`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch(`${baseURL}/transactions/stat/weekly`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch(`${baseURL}/transactions/stat/monthly`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch(`${baseURL}/transactions/stat/daily`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch(`${baseURL}/transactions/stat/comparison`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ])
      ...
      const [transactionsData, weeklyDataJson, monthlyDataJson, dailyDataJson, comparisonDataJson] = await Promise.all([
  ...
  // Generate comparison data using available data
  const generateComparisonData = () => {
    // Daily comparison - use last 2 days
    const dailyComparison = dailyData.length >= 2 ? {
      current: dailyData[dailyData.length - 1],
      previous: dailyData[dailyData.length - 2],
      incomeChange: dailyData[dailyData.length - 1].income - dailyData[dailyData.length - 2].income,
      expenseChange: dailyData[dailyData.length - 1].expense - dailyData[dailyData.length - 2].expense,
      netChange: dailyData[dailyData.length - 1].net - dailyData[dailyData.length - 2].net
    } : null

    // Weekly comparison - use comparison API data for today vs this week
    const weeklyComparison = comparisonData ? {
      current: {
        income: comparisonData.thisWeek.income,
        expense: comparisonData.thisWeek.expense,
        net: comparisonData.thisWeek.net,
        transactionCount: comparisonData.thisWeek.transactionCount
      },
      previous: {
        income: comparisonData.today.income,
        expense: comparisonData.today.expense,
        net: comparisonData.today.net,
        transactionCount: comparisonData.today.transactionCount
      },
      incomeChange: comparisonData.thisWeek.income - comparisonData.today.income,
      expenseChange: comparisonData.thisWeek.expense - comparisonData.today.expense,
      netChange: comparisonData.thisWeek.net - comparisonData.today.net
    } : null

    // Monthly comparison - use comparison API data for this week vs this month
    const monthlyComparison = comparisonData ? {
      current: {
        income: comparisonData.thisMonth.income,
        expense: comparisonData.thisMonth.expense,
        net: comparisonData.thisMonth.net,
        transactionCount: comparisonData.thisMonth.transactionCount
      },
      previous: {
        income: comparisonData.thisWeek.income,
        expense: comparisonData.thisWeek.expense,
      ...
      incomeChange: comparisonData.thisMonth.income - comparisonData.thisWeek.income,
      expenseChange: comparisonData.thisMonth.expense - comparisonData.thisWeek.expense,
      netChange: comparisonData.thisMonth.net - comparisonData.thisWeek.net
    } : null
  ...
  const comparisonChartData = generateComparisonData()
          ...
          >
            <div className="flex items-start justify-between mb-6">
              <div>
                ...
                <p className="text-muted-foreground text-lg">Unlock powerful insights into your financial journey</p>
              </div>
              <div className="flex items-center gap-2">
            ...
            <TabsContent value="trends">
              <Card className="glass border-2 rounded-2xl overflow-hidden">
                <CardHeader>
                  ...
                  <CardDescription>Track your income, expenses, and net over time</CardDescription>
                </CardHeader>
                <CardContent>
            ...
            <TabsContent value="comparison">
              <Card className="glass border-2 rounded-2xl overflow-hidden">
                <CardHeader>
                  ...
                  <CardDescription>Compare financial performance across different time periods</CardDescription>
                </CardHeader>
                <CardContent>
                  <Tabs defaultValue="daily" className="w-full">
                    ...
                    <TabsContent value="daily">
                      {comparisonChartData.dailyComparison ? (
                        ...
                        <div className="h-[400px] flex flex-col items-center justify-center text-muted-foreground">
                          ...
                          <p>Insufficient daily data for comparison</p>
                    ...
                    <TabsContent value="weekly">
                      {comparisonChartData.weeklyComparison ? (
                        ...
                        <div className="h-[400px] flex flex-col items-center justify-center text-muted-foreground">
                          <BarChart3 className="w-16 h-16 mb-4 opacity-20" />
                          <p>Weekly comparison data not available</p>
                          <p className="text-sm mt-2">Complete API data required</p>
                        </div>
                      )}
                    </TabsContent>
                    ...
                    <TabsContent value="monthly">
                      {comparisonChartData.monthlyComparison ? (
                        ...
                        <div className="h-[400px] flex flex-col items-center justify-center text-muted-foreground">
                          <BarChart3 className="w-16 h-16 mb-4 opacity-20" />
                          <p>Monthly comparison data not available</p>
                          <p className="text-sm mt-2">Complete API data required</p>
                        </div>
                      )}
                    </TabsContent>
            ...
            <TabsContent value="insights">
              <Card className="glass border-2 rounded-2xl overflow-hidden">
                <CardHeader>
                  ...
                  <CardDescription>Deep financial analysis and predictive insights</CardDescription>
                </CardHeader>
                <CardContent>
